...
using AfricaPeaceEnablers.Data;
    ...
    public class SignalDashboardService : ISignalDashboardService
    {
        ...
        public async Task<ResultResponseDto<PeaceStressTestDashboardDto>> GetPeaceStressTestDashboard(int countryID, int year, int userId)
        {
            try
            {
                ...
                var APEMScores = await LoadCountryAPEMScores(countryID, year);
                ...
                var primarySignals = BuildSignalCards(primaryMappings, layers, currentResults, previousResults, accessibleLayerIds, APEMScores.Current);
                ...
                primarySignals.Insert(0, new SignalCardDto
                {
                    ...
                    Code = "APEM Score",
                    Name = "Country Score",
                    Value = APEMScores.Current,
                    Delta = APEMScores.Delta,
                    Condition = CommonStaticMethods.GetConditionByScore(APEMScores.Current)
                });

                var secondarySignals = BuildSignalCards(secondaryMappings, layers, currentResults, previousResults, accessibleLayerIds, APEMScores.Current);
                ApplyAPEMToLayerCard(primarySignals, APEMScores, layers);
                ApplyAPEMToLayerCard(secondarySignals, APEMScores, layers);
                var APEMLayer = layers.Values.FirstOrDefault(x => x.LayerCode.Equals("APEM", StringComparison.OrdinalIgnoreCase));

                var APEMInterpretation = ResolveInterpretation(
                    APEMLayer,
                    APEMLayer != null && currentResults.TryGetValue(APEMLayer.LayerID, out var APEMResult)
                        ? APEMResult.InterpretationId
                        : null);
                var APEMCondition = CommonStaticMethods.GetConditionByScore(APEMScores.Current);

                var narratives = primarySignals
                    .Where(x => !x.LayerCode.Equals("APEM", StringComparison.OrdinalIgnoreCase))
                ...
                var response = new PeaceStressTestDashboardDto
                {
                    CountryID = countryID,
                    Year = year,
                    APEM = APEMScores.Current,
                    CountryScore = APEMScores.Current,
                    APEMDirectionalMovement = APEMScores.Delta,
                    APEMCondition = APEMCondition,
                    APEMDescriptor = APEMInterpretation?.Descriptor ?? string.Empty,
                    APEMStrategicAction = APEMInterpretation?.StrategicAction ?? string.Empty,
        ...
        private async Task<CountryAPEMScores> LoadCountryAPEMScores(int countryID, int year)
        {
            ...
            decimal? APEMOverride = null)
        {
            ...
            foreach (var mapping in mappings)
            {
                ...
                if (APEMOverride.HasValue &&
                    layer.LayerCode.Equals("APEM", StringComparison.OrdinalIgnoreCase))
                {
                    value = APEMOverride.Value;
                }
        ...
        private static void ApplyAPEMToLayerCard(
            List<SignalCardDto> cards,
            CountryAPEMScores APEMScores,
            IReadOnlyDictionary<int, AnalyticalLayer> layers)
        {
            var APEMCard = cards.FirstOrDefault(x => x.LayerCode.Equals("APEM", StringComparison.OrdinalIgnoreCase));
            ...
            APEMCard.Value = APEMScores.Current;
            APEMCard.Delta = APEMScores.Previous.HasValue ? APEMScores.Delta : null;

            if (layers.TryGetValue(APEMCard.LayerID, out var APEMLayer))
            {
                var interpretation = ResolveInterpretation(APEMLayer, APEMCard.InterpretationID > 0 ? APEMCard.InterpretationID : null)
                                     ?? MatchInterpretationByValue(APEMLayer, APEMScores.Current);

                if (interpretation != null)
                {
                    APEMCard.Condition = interpretation.Condition;
                    APEMCard.Descriptor = interpretation.Descriptor;
                    APEMCard.StrategicAction = interpretation.StrategicAction;
                    APEMCard.Narrative = interpretation.Descriptor;
                    APEMCard.InterpretationID = interpretation.InterpretationID;
                }
                else
                {
                    APEMCard.Condition = ResolveConditionByValue(APEMLayer, APEMScores.Current);
                }
            }
        }
        ...
        private sealed class CountryAPEMScores
        {
