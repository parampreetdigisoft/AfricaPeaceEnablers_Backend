...
using AfricaPeaceEnablers.Views.EmailModels;
    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<ResultResponseDto<object>> ForgotPassword(string email)
        {
            try
            {
                ...
                else
                {
                    ...
                    var sub = "Password Update Link – Africa Peace Enablers Matrix Platform";
                    var model = new EmailInvitationSendRequestDto
                    {
                        ...
                        ApplicationUrl = url,
                        MsgText= "A request was made to update the password for your Africa Peace Enablers Matrix (APEM) account. To proceed, please use the secure link below:",
                        IsShowBtnText=true,
                        IsLoginBtn=false,
        ...
        public async Task<ResultResponseDto<object>> InviteUser(InviteUserDto inviteUser)
        {
            try
            {
                ...
                string sub = $"{roleName} Access Granted – Africa Peace Enablers Matrix Platform";
        ...
        //               msgText = $"You are receiving this email because {invitedUser?.FullName} recently requested country assignment ({invitedCountryNames}) for your APEM account.";
        //           }
        ...
        //               if (isMailSent)
        //               {
        //                   msgText += $" Additionally, you no longer have access to the countries ({deleteCountryNames}) for your APEM account.";
        //               }
        //               else
        //               {
        //                   msgText = $"You are receiving this email because {invitedUser?.FullName} recently removed your access to the following countries ({deleteCountryNames}) for your APEM account.";
        //               }
        //               isMailSent = true;
        //           }
        ...
        public async Task<ResultResponseDto<object>> UpdateInviteUser(UpdateInviteUserDto inviteUser)
        {
            try
            {
                ...
                if (countriesToAdd.Count > 0)
                {
                    ...
                    msgText = $"You are receiving this email because {invitedUser?.FullName} recently requested country assignment ({addedNames}) for your APEM account.";
                }
                ...
                if (countriesToDelete.Count > 0)
                {
                    ...
                    msgText = isMailSent
                        ? msgText + $" Additionally, you no longer have access to the countries ({removedNames}) for your APEM account."
                        : $"You are receiving this email because {invitedUser?.FullName} recently removed your access to the following countries ({removedNames}) for your APEM account.";
                    isMailSent = true;
                }
            ...
            try
            {
                ...
                foreach (var inviteUser in inviteUserList.users)
                {
                    ...
                    if (countriesToAdd.Count() > 0)
                    {
                        ...
                        string sub = $"{inviteUser.Role.ToString()} Access Granted – Africa Peace Enablers Matrix Platform";
                        var model = new EmailInvitationSendRequestDto
                        {
                ...
                else
                {
                    ...
                    if (assessment != null)
                    {
                        ...
                        var model = new EmailInvitationSendRequestDto
                        {
                            ...
                            MsgText = $"You are receiving this email because user {user?.FullName} recently requested to update assessment of {country?.CountryName} from their Africa Peace Enablers Matrix account.",
                            BtnText = "Give Access",
                            Mail = _appSettings.AdminMail
                        };
        ...
        public async Task<ResultResponseDto<string>> SendTwoFactorOTPAsync(User user)
        {
            try
            {
                ...
                var model = new EmailInvitationSendRequestDto
                {
                    ...
                    IsShowBtnText = false,
                    Mail = _appSettings.AdminMail,
                    DescriptionAboutBtnText = "You are receiving this email because a login attempt was made to your APEM account. " +
            ...
            try
            {
                ...
                if (requestDto.Email != user.Email)
                {
                    ...
                    var emailModel = new EmailInvitationSendRequestDto
                    {
                        ...
                        ApiUrl = _appSettings.ApiUrl,
                        ApplicationUrl = _appSettings.PublicApplicationUrl,
                        MsgText = "A request was made to update the Email for your Africa Peace Enablers Matrix (APEM) account. Please verify your email or reset your password.",
                        Mail = _appSettings.AdminMail,
                        BtnText = "Verify",
                        DescriptionAboutBtnText = "Please verify your email address by clicking the button above."
                    };
