...
namespace AfricaPeaceEnablers.Common.Implementation
{
    public partial class PdfGeneratorService : Interface.IPdfGeneratorService
    {
        ...
        void RenderPillarRadarCard(IContainer container, List<PillarChartItem> pillars)
        {
            container
                .Background(Colors.White)
                .Border(1).BorderColor("#C5CBB5")
                .Padding(4)
                .Column(col =>
                {
                    col.Item().AlignCenter()
                        .Text("Pillar Performance Radar")
                        .FontSize(10).Bold().FontColor("#1B3022");
        ...
        static void PaintSpiderChart(SKCanvas canvas, Size size, List<PillarChartItem> pillars)
        {
            ...
            using var fillPaint = new SKPaint
            {
                ...
                Color = SKColor.Parse("#3C5A64").WithAlpha(55),
                IsAntialias = true
            };
            ...
            container
                ...
                .Column(col =>
                {
                    col.Item()
                        ...
                        .FontSize(9).Bold().FontColor("#1B3022");

                    col.Item().PaddingTop(7).Row(row =>
                    {
                        DashboardStatCard(row.RelativeItem(),
                            green.ToString(), "Performing ≥70%", "#EEF1E8", "#6B7F62");
                        row.ConstantItem(8);
                        DashboardStatCard(row.RelativeItem(),
                        ...
                        row.ConstantItem(8);
                        DashboardStatCard(row.RelativeItem(),
                            total.ToString(), "Total KPIs", "#F7F4EE", "#1B3022");
                    });
                });
        }
        ...
        static void PaintKpiSparkline(SKCanvas canvas, Size size, List<KpiChartItem> kpis)
        {
            ...
            // Grid lines
            using var gp = new SKPaint { Color = SKColor.Parse("#F7F4EE"), StrokeWidth = 0.7f };
            ...
            using var shader = SKShader.CreateLinearGradient(
                ...
                new[] { SKColor.Parse("#6B7F62").WithAlpha(95),
                SKColor.Parse("#6B7F62").WithAlpha(8) },
            ...
            {
                Color = SKColor.Parse("#6B7F62").WithAlpha(140),
                StrokeWidth = 0.9f,
            ...
            int total, int green, int amber, int red, float avg)
        {
            container
                ...
                .Row(row =>
                {
                    KpiStatPill(row.RelativeItem(), total.ToString(), "Total KPIs", "#6B7F62", "#6B7F6225");
                    row.ConstantItem(6);
                    KpiStatPill(row.RelativeItem(), green.ToString(), "Performing ≥ 70 %", "#6B7F62", "#6B7F6225");
                    row.ConstantItem(6);
                    ...
                    KpiStatPill(row.RelativeItem(), $"{avg:F1}%", "Average Score",
                        avg >= 70 ? "#6B7F62" : avg >= 40 ? "#FFC107" : "#EF5350",
                        "#6B7F6225");
                });
        }
        ...
        void DrawKpiGroupSection(IContainer container, List<KpiChartItem> group, int offset, bool isAllCountries = false)
        {
            ...
            .Column(col =>
            {
                ...
                col.Item().Height(1).Background("#C5CBB5");
        ...
        void DrawKpiBarChart(IContainer container, List<KpiChartItem> data, int offset)
        {
            container
                ...
                .Canvas((canvas, size) =>
                {
                    ...
                    {
                        Color = SKColor.Parse("#6B7F62").WithAlpha(100),
                        StrokeWidth = 0.9f,
                        PathEffect = SKPathEffect.CreateDash(new[] { 4f, 3f }, 0),
                        IsAntialias = true
                    };
        ...
        void DrawKpiCard(ColumnDescriptor card, KpiChartItem kpi, int num)
        {
            ...
            card.Item()
                ...
                .Column(inner =>
                {
                    ...
                    {
                        inner.Item()
                             .Background("#F7F4EE")                      // very pale green-grey
        ...
        static void DrawPillarHorizontalBars(IContainer container, List<PillarChartItem> data)
        {
            ...
            container
                .Background(Colors.White)
                .Border(1).BorderColor("#D8DCC8")
                .Padding(14)
                .Column(col =>
                {
                    col.Item().PaddingBottom(8)
                        .Text("Pillar Overview").FontSize(11).Bold().FontColor("#1B3022");
            ...
            IContainer container, float avg, PillarChartItem best, PillarChartItem worst)
        {
            container.Row(row =>
            {
                ...
                row.RelativeItem(2)
                    ...
                    {
                        c.Item().AlignCenter()
                            .Text("Average Score").FontSize(9).FontColor("#A3AD91");
                        c.Item().AlignCenter()
                            .Text($"{avg:F1}")
                            .FontSize(22).Bold()
                            .FontColor(GetBarColor(avg) == "#6B7F62" ? "#A3AD91"
                ...
                row.RelativeItem(3)
                    ...
                    .Column(c =>
                    {
                        ...
                        c.Item().PaddingTop(4)
                            .Text($"{best.Value:F1}").FontSize(16).Bold().FontColor("#6B7F62");
                    });
            ...
            string? pillarName)
        {
            var logoPath = Path.Combine(
                ...
                "wwwroot/assets/images/apem.png");

            container.Column(column =>
            {
                column.Item().Background("#1B3022").Padding(8).Row(row =>
                {
                    // Left content
                ...
                // Divider
                column.Item().LineHorizontal(1).LineColor("#D8DCC8");
            });
        }        
        ...
        void PillarComposeHeader(IContainer container, AiCountryPillarResponse data)
        {
            ...
            container.Column(column =>
            {
                column.Item().Background("#1B3022").Padding(12).Row(row =>
                {
                    // Left content
                    row.RelativeItem().Column(col =>
                    {
        ...
        void CountrySummeryComposeContent(IContainer container, AiCountrySummeryDto data, UserRole userRole, bool isAllCountries=false)
        {
            container.PaddingTop(4).Column(column =>
            {
                ...
                column.Item().PaddingTop(10).Element(c =>
                    PillarContentSection(c, "Executive Summary", SanitizeText(data.EvidenceSummary), "#1B3022"));
                ...
                if (!isAllCountries) {
                    ...
                    if (!string.IsNullOrEmpty(data.KeyDevelopments))
                        column.Item().PaddingTop(8).Element(c =>
                            PillarContentSection(c, "Key Developments", SanitizeText(data.KeyDevelopments), "#3C5A64"));
                    if (!string.IsNullOrEmpty(data.CriticalRisks))
                        column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Critical Risks", SanitizeText(data.CriticalRisks), "#3C5A64"));
                    if (!string.IsNullOrEmpty(data.Gaps))
                        column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Gaps", SanitizeText(data.Gaps), "#A3AD91"));
                    ...
                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Structural Evidence", SanitizeText(data.StructuralEvidence), "#D8DCC8"));

                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Operational Evidence", SanitizeText(data.OperationalEvidence), "#D8DCC8"));

                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Outcome Evidence", SanitizeText(data.OutcomeEvidence), "#ffe6cc"));

                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Perception Evidence", SanitizeText(data.PerceptionEvidence), "#EEF1E8"));
                    ...
                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Temporal Scope", SanitizeText(data.TemporalScope), "#EEF1E8"));

                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Distortion Screening", SanitizeText(data.DistortionScreening), "#f2d9e6"));

                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Relational Integrity", SanitizeText(data.RelationalIntegrity), "#EEF1E8"));
                    ...
                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Narrative Shock", SanitizeText(data.NarrativeShock), "#EEF1E8"));
                    //column.Item().PageBreak();

                    //column.Item().PaddingTop(8).Element(c =>
                    //    PillarContentSection(c, "Overall Stress Resilience", SanitizeText(data.OverallStressResilience), "#EEF1E8"));
                    ...
                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Non Compensation Note", SanitizeText(data.NonCompensationNote), "#EEF1E8"));
                    ...
                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Cross-Pillar System Dynamics", SanitizeText(data.CrossPillarPatterns), "#6B7F62"));

                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Institutional Capacity Assessment", SanitizeText(data.InstitutionalCapacity), "#6B7F62"));
                    ...
                    column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Strategic Policy Priorities", SanitizeText(data.StrategicRecommendation), "#6B7F62"));

                    if (!string.IsNullOrEmpty(data.KeyFindings))
                        column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Key Findings", SanitizeText(data.KeyFindings), "#1B3022"));
                }                
            ...
            container.PaddingTop(4).Column(column =>
            {
                if (!isAllCountries)
                {
                    ...
                    if (!string.IsNullOrEmpty(data.Recommendations))
                        column.Item().PaddingTop(8).Element(c =>
                        PillarContentSection(c, "Recommendations", SanitizeText(data.Recommendations), "#1B3022"));
                }
            });
        }
     ...
     IContainer container, AiCountryPillarResponse data, UserRole userRole)
        {
            container.PaddingTop(8).Column(column =>
            {
                ...
                // =====================================================
                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Structural Evidence", SanitizeText(data.StructuralEvidence), "#3C5A64"));

                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Operational Evidence", SanitizeText(data.OperationalEvidence), "#3C5A64"));

                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Outcome Evidence", SanitizeText(data.OutcomeEvidence), "#A3AD91"));

                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Perception Evidence", SanitizeText(data.PerceptionEvidence), "#C5CBB5"));
                ...
                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Temporal Scope", SanitizeText(data.TemporalScope), "#3C5A64"));

                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Distortion Screening", SanitizeText(data.DistortionScreening), "#5A6B5E"));

                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Relational Integrity", SanitizeText(data.RelationalIntegrity), "#A3AD91"));
                ...
                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Political Shock", SanitizeText(data.StressPoliticalShock), "#A8905C"));

                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Economic Shock", SanitizeText(data.StressEconomicShock), "#BFA16D"));

                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Narrative Shock", SanitizeText(data.StressNarrativeShock), "#D4C4A0"));
                ...
                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Inequality Adjustment", SanitizeText(data.InequalityAdjustment), "#1B3022"));

                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Opacity Risk", SanitizeText(data.OpacityRisk), "#6B7F62"));

                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Non Compensation Note", SanitizeText(data.NonCompensationNote), "#A3AD91"));
                ...
                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Geographic Equity Note", SanitizeText(data.GeographicEquityNote), "#6B7F62"));
                ...
                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Institutional Assessment", SanitizeText(data.InstitutionalAssessment), "#6B7F62"));

                column.Item().PaddingTop(8).Element(c =>
                    PillarContentSection(c, "Analytical Foundations and Data Integration", SanitizeText(data.DataGapAnalysis), "#A3AD91"));
        ...
        void CountryProgressSection(IContainer container, AiCountrySummeryDto data, UserRole userRole)
        {
            container
                ...
                .Column(column =>
                {
                    ...
                    {
                        // Score Section
                        PillarProgressBar(col, "Total Score", data.AIProgress, "#6B7F62");
                        ...
                        RankRowModern(col, "Global Rank", data.Rank, data.TotalCountry, "#6B7F62");

                        col.Item().PaddingTop(2);

                        RankRowModern(col, $"{data.Region} Region Rank", data.RegionRank, data.RegionTotalCountry, "#3C5A64");
                    });
                });
        }
            ...
            IContainer container, AiCountryPillarResponse data, UserRole userRole, bool isCity = false)
        {
            container
                ...
                .Column(column =>
                {
                    ...
                    PillarProgressBar(column, "Score", data.AIProgress, "#6B7F62");
                });
        }
        ...
        static SKColor GetColor(float value)
        {
            ...
            else if(value >= 20) return SKColor.Parse("#6B7F62");
            return SKColor.Parse("#6B7F62");
        }
        static string GetBarColor(float value)
        {
            ...
            else if (value >= 20) return "#6B7F62";
            return "#6B7F62";
        }
    ...
    {   
        ...
        {
            "#1B3022","#6B7F62","#6B7F62","#BFA16D","#F5A623",
            "#E05252","#5A6B5E","#3C5A64","#6B7F62","#FB8C00",
            "#3C5A64","#8D6E63","#E91E63","#607D8B"
        };
            ...
            AiCountrySummeryDto countryDetails)
        {
            ...
            container.Padding(16).Column(col =>
            {
                ...
                col.Item().Text("Population Size by Country")
                    .FontSize(11).Bold().FontColor("#1B3022");
            ...
            long maxPop)
        {
            ...
            for (int i = 0; i < countries.Count; i++)
            {
                ...
                DrawCanvasText(canvas, country.CountryName, 4, y + 5, 9,
                    isMain ? "#1B3022" : "#444444", bold: isMain);
        ...
        private static string GetSegmentColor(string category)
        {
            return category switch
            {
                ...
                "Upper-Middle Income" => "#A3AD91", // blue-green
                "High Income" => "#6B7F62",         // strong green
                _ => "#999999"
            };
        }
            ...
            container.Padding(16).Column(col =>
            {
                ...
                col.Item().Height(130).Canvas((canvas, size) =>
                {
                    ...
                    for (int i = 0; i < categoryOrder.Length; i++)
                    {
                        ...
                        DrawCanvasText(canvas, $"{avg:F1}", x + barAreaW / 2 - 10, 100 - barH - 14, 9, "#1B3022", true);
                    ...
                    foreach (var (label, countries) in segments)
                    {
                        foreach (var country in countries.OrderByDescending(c => GetLatestScoreOrZero(c)))
                        {
                            ...
                            table.Cell().Background(rowBg).BorderBottom(0.5f).BorderColor("#e0e0e0")
                                ...
                                .FontColor(isMain ? "#1B3022" : "#333333");
                            table.Cell().Background(rowBg).BorderBottom(0.5f).BorderColor("#e0e0e0")
            ...
            AiCountrySummeryDto countryDetails)
        {
            ...
            container.Padding(16).Column(col =>
            {
                ...
                // ── Hero rank banner ──────────────────────────────────────────
                col.Item().Background("#1B3022").Padding(14).Row(row =>
                {
                ...
                // ── Full ranking table ────────────────────────────────────────
                col.Item().Text("Full Country Ranking").FontSize(11).Bold().FontColor("#1B3022");
            ...
            AiCountrySummeryDto countryDetails)
        {
            ...
            container.Padding(16).Column(col =>
            {
                ...
                col.Item().Text("6.1  Historical Score Trend")
                    .FontSize(12).Bold().FontColor("#1B3022");
            ...
            List<(int Year, float Avg)> peerAvg)
        {
            container.Table(table =>
            {
                ...
                // Header
                table.Cell().Background("#1B3022").Padding(5)
                    .Text("Metric").FontSize(8).Bold().FontColor(Colors.White);
                foreach (var yr in years)
                    table.Cell().Background("#1B3022").Padding(5).AlignRight()
            ...
            container.Padding(16).Column(col =>
            {
                ...
                col.Item().Table(table =>
                {
                    ...
                    foreach (var yr in allYears)
                        table.Cell().Background("#1B3022").Padding(5).AlignCenter()
                            .Text(yr.ToString()).FontSize(8).Bold().FontColor(Colors.White);
                    ...
                    foreach (var (pillar, pi) in pillars.Select((p, i) => (p, i)))
                    {
                        ...
                        foreach (var yr in allYears)
                        {
                            ...
                            string cellBg = !hasData ? "#f0f0f0"
                                : InterpolateColor("#ffffff", "#1B3022", score / 100f);
                ...
                {
                    col.Item().PaddingTop(6)
                        .Text("Pillar Trend Highlights").FontSize(11).Bold().FontColor("#1B3022");
                    ...
                    col.Item().Row(row =>
                    {
                        ...
                        {
                            c.Item().Text("(+) Most Improved").FontSize(9).Bold().FontColor("#6B7F62");
                            foreach (var pd in pillarDeltas.Take(3))
                                c.Item().Row(r =>
                                {
                                    ...
                                    r.ConstantItem(44).AlignRight()
                                        .Text($"+{pd.Delta:F1}").FontSize(8).Bold().FontColor("#6B7F62");
                                });
                        });
            ...
            List<(int Year, float Avg)> peerAvg)
        {
            ...
            canvas.DrawPath(peerPath,
                new SKPaint { Color = SKColor.Parse("#6B7F62").WithAlpha(40), IsAntialias = true });
        ...
        void DrawDonutChart(SKCanvas canvas, Size size, List<(string Label, float Value)> segments)
        {
            ...
            // Centre count
            DrawCanvasText(canvas, $"{segments.Count}", cx - 8, cy - 8, 14, "#1B3022", bold: true);
        ...
        void DrawInsightBand(IContainer container, string text)
        {
            container.Background("#e8f5e9").Padding(8)
                .Text(text).FontSize(8.5f).FontColor("#1B3022");
        }
        ...
        static string ScoreColor(float score) =>
            score >= 70 ? "#6B7F62" : score >= 40 ? "#f5a623" : "#e05252";
